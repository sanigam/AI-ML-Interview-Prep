...
A) The global minimum of f  
B) Steepest increase of f  
C) Steepest decrease of f  
D) Zero change in f
...
C) SGD guarantees finding the global minimum  
D) SGD does not use a learning rate

---

**Q4. A function is convex if:**

A) Every local minimum is a global minimum, and a line segment between any two points lies on or above the function  
B) It has exactly one local minimum  
...
A) The second-order partial derivatives  
B) The first-order partial derivatives  
...
A) The function achieves its global minimum  
B) The gradient is zero, but the point is neither a local minimum nor a local maximum  
C) All second derivatives are positive  
D) The gradient is non-zero

---

**Q7. The chain rule is essential in deep learning because:**

A) It enables backpropagation by computing gradients through nested function compositions  
B) It determines the number of layers in a network  
...
C) Random search and grid search  
D) Newton's method and gradient descent

---

**Q9. L2 regularization adds λ||θ||² to the loss function. In terms of optimization, this:**

A) Removes the gradient entirely  
B) Makes the problem non-convex  
C) Makes the loss function non-differentiable  
D) Adds curvature to the loss surface, shrinking parameters toward zero

---

**Q10. Learning rate warmup is a technique where:**

A) The learning rate starts small and gradually increases before following a decay schedule  
B) The learning rate is fixed throughout training  
C) The learning rate starts high and decreases linearly  
...
A) Quadratically near the optimum, but is computationally expensive  
B) Linearly, same as gradient descent  
C) Only for non-convex functions  
D) Slower than SGD in all cases

---

**Q13. What is the purpose of gradient clipping?**

A) To remove features with small gradients  
B) To increase the learning rate automatically  
C) To make the loss function convex  
D) To prevent exploding gradients by capping gradient magnitudes

---

**Q14. In the context of neural networks, a loss function must be:**

A) Constant across all training examples  
B) Convex with respect to the inputs  
C) Independent of the model architecture  
D) Differentiable with respect to the model parameters (at least almost everywhere)
...
C) It guarantees convergence in fewer epochs  
