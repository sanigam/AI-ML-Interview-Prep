...
B) Statistical methods like Z-score  
C) Unsupervised methods like Isolation Forest  
...
A) It assumes a normal distribution and fails on multivariate data with skewed distributions  
...
A) It eliminates the need for covariance matrix estimation  
B) It is faster to compute in high dimensions  
C) It requires no assumptions about data distribution  
D) It accounts for correlations between variables and different scales
...
A) Anomalies can be isolated with fewer random splits than normal points  
B) Anomalies form dense clusters that are easy to identify  
C) Anomalies require more splits to be isolated than normal points  
D) Anomalies have higher local density than normal points
...
B) The threshold for reconstruction error  
C) The expected proportion of anomalies in the dataset  
...
A) Contextual anomalies where points are normal globally but anomalous relative to their local neighborhood  
B) Anomalies in high-dimensional data with >100 dimensions  
C) Collective anomalies in time series data  
D) Global point anomalies where anomalous points deviate from the overall mean
...
B) Normal data can be reconstructed with low error; anomalies (unseen during training) will have high reconstruction error  
C) Anomalies are more compressible than normal data  
...
C) VAEs work only on time series data  
D) VAEs eliminate the need for threshold selection
...
B) Isolation Forest  
C) One-Class SVM  
D) STL (Seasonal and Trend decomposition using Loess)
...
A) It handles extreme class imbalance better and is more informative when anomalies represent <1% of data  
B) It automatically selects the optimal threshold  
C) It is faster to compute  
D) It does not require labeled test data
...
A) Adjust class weights, use anomaly-specific algorithms, employ cost-sensitive learning, and validate with stratified sampling  
B) Report accuracy as the primary evaluation metric  
C) Use standard binary classification with balanced datasets  
D) Oversample all anomalies using SMOTE regardless of training distribution changes

---

## Answer Key

**Question 1: D) Point anomaly**
...
**Question 2: A) Collective anomaly**
...
**Question 3: D) Supervised classification methods**
...
**Question 4: A) It assumes a normal distribution and fails on multivariate data with skewed distributions**
...
**Question 5: D) It accounts for correlations between variables and different scales**
...
**Question 6: A) Anomalies can be isolated with fewer random splits than normal points**
...
**Question 7: C) The expected proportion of anomalies in the dataset**
...
**Question 8: A) Contextual anomalies where points are normal globally but anomalous relative to their local neighborhood**
...
**Question 11: B) Normal data can be reconstructed with low error; anomalies (unseen during training) will have high reconstruction error**
...
**Question 13: D) STL (Seasonal and Trend decomposition using Loess)**
...
**Question 14: A) It handles extreme class imbalance better and is more informative when anomalies represent <1% of data**
...
**Question 15: A) Adjust class weights, use anomaly-specific algorithms, employ cost-sensitive learning, and validate with stratified sampling**
