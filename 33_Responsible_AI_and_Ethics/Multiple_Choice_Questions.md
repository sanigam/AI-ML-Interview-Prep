...
C) The algorithm is poorly coded  
...
A) Randomly assigning feature importance  
B) Using game-theoretic Shapley values to assign each feature a contribution score for a specific prediction  
C) Removing the model entirely  
D) Only explaining linear models

---

**Q4. LIME (Local Interpretable Model-agnostic Explanations) works by:**

A) Replacing the model with a rule-based system  
B) Only using decision trees  
C) Perturbing inputs around a prediction and fitting a simple interpretable model locally to approximate the black-box behavior  
D) Globally retraining the model
...
C) Model predictions after training, such as using different decision thresholds per group  
...
A) Only the model's file size  
B) Only the training code  
C) Model details, intended use, performance across groups, limitations, ethical considerations, and training data characteristics  
D) Only the model's accuracy

---

**Q8. Equalized odds requires that the model has:**

A) Zero false positives only  
B) Perfect accuracy  
C) The same prediction rate for all groups  
D) Equal true positive rates AND equal false positive rates across protected groups
...
C) Features are perfectly correlated  
D) Certain groups are underrepresented, causing the model to perform worse for those groups

---

**Q10. Grad-CAM explains image classifier decisions by:**

A) Only working with text models  
B) Retraining the model  
C) Randomly highlighting pixels  
D) Visualizing which image regions most influenced the prediction using gradient-weighted activation maps

---

**Q11. The EU AI Act classifies AI systems into risk categories requiring:**

A) Different levels of transparency, documentation, and oversight based on the risk level (unacceptable, high, limited, minimal)  
B) No regulations for any AI  
C) Only voluntary guidelines  
D) All AI to be banned

---

**Q12. Adversarial debiasing is an in-processing technique that:**

A) Removes all features  
...
C) Only works with GANs  
D) Adds more biased data

---

**Q13. Differential privacy in ML ensures that:**

A) The presence or absence of any single individual's data has minimal impact on the model's outputs, protecting individual privacy  
B) Only large datasets can be used  
C) The model's weights are encrypted  
D) All data is public

---

**Q14. The right to explanation under GDPR gives individuals:**

A) The right to retrain the model  
B) The right to delete the model  
C) The right to meaningful information about the logic behind automated decisions that significantly affect them  
D) Access to all training data
...
B) The model always predicts 50%  
C) When the model predicts a probability (e.g., 80%), the actual positive rate should be approximately 80% across all groups  
D) All groups have the same features
