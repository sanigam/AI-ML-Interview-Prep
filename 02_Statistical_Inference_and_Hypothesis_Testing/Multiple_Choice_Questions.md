...
A) If we repeated the sampling procedure many times, approximately 95% of the constructed intervals would contain the true parameter  
B) There is a 95% probability that the true parameter lies within this specific interval  
...
A) If the null hypothesis were true, there is a 3% probability of observing results this extreme or more extreme  
B) There is a 3% probability that the null hypothesis is true  
C) The effect size is 3%  
D) There is a 97% probability that the treatment works
...
B) The p-value is less than the significance level  
C) You reject the null hypothesis when it is actually true  
...
A) The probability of correctly failing to reject a true null hypothesis  
B) The probability of obtaining a p-value less than 0.05  
...
C) Increasing the number of hypothesis tests performed  
...
A) To ensure the variance is always positive  
B) To make the computation simpler  
C) To correct for the bias introduced by using the sample mean instead of the population mean  
D) Because the sample always has one fewer observation than the population

---

**Q8. Maximum Likelihood Estimation (MLE) finds the parameter value that:**

A) Maximizes the posterior probability of the parameter given the data  
B) Minimizes the sum of squared errors  
C) Maximizes the probability of the observed data given the parameter  
D) Minimizes the prior probability of the parameter
...
A) Chi-square test  
B) Two-sample t-test repeated 6 times  
C) One-way ANOVA  
D) Paired t-test
...
A) It measures the Type I error rate  
B) It equals the sample size required for the test  
C) Its inverse provides a lower bound on the variance of any unbiased estimator (Cramér-Rao bound)  
D) It determines the maximum possible bias of an estimator
...
A) It decreases the effect size  
B) It makes the test non-parametric  
C) It increases the required sample size  
D) It inflates the actual Type I error rate above the nominal α level

---

**Q15. A chi-square test for independence is used to determine:**

A) Whether the means of two groups are equal  
B) Whether a continuous variable follows a normal distribution  
C) Whether the variance of a sample equals a hypothesized value  
D) Whether two categorical variables are independent of each other
