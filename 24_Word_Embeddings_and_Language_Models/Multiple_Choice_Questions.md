...
A) Applying PCA to TF-IDF vectors  
B) Counting word co-occurrences in a matrix  
C) Predicting context words from a given center word  
D) Predicting the center word from its context words
...
B) All word analogies are perfectly solved  
C) Embeddings are random vectors  
D) Word embeddings capture semantic relationships as linear vector operations

---

**Q3. GloVe differs from Word2Vec primarily because it:**

A) Combines global co-occurrence statistics with local context prediction  
B) Uses only local context windows  
C) Requires labeled training data  
D) Cannot handle rare words

---

**Q4. FastText handles out-of-vocabulary (OOV) words by:**

A) Ignoring them during training  
B) Representing words as sums of character n-gram embeddings, allowing OOV words to be constructed from known subwords  
C) Assigning them a zero vector  
...
A) They cannot be used for downstream tasks  
B) They assign one fixed vector per word regardless of context  
C) They use smaller vocabularies  
D) They compute different word representations depending on the surrounding context

---

**Q6. BERT's masked language modeling (MLM) objective masks tokens with the strategy:**

A) 80% [MASK], 10% random token, 10% unchanged  
B) 100% replaced with [MASK]  
C) All tokens are masked simultaneously  
D) 50% [MASK], 50% unchanged
...
A) Replaces the expensive full softmax with a simplified objective using k randomly sampled negative examples  
B) Increases the vocabulary size  
C) Only trains on words with positive sentiment  
D) Removes negative sentiment words from the corpus

---

**Q9. Subword tokenization (BPE, WordPiece) is crucial for transformers because it:**

A) Always produces single-token words  
...
A) Removing NSP, using dynamic masking, training longer on more data, and tuning hyperparameters carefully  
B) Reducing the model size to half  
C) Switching from transformers to RNNs  
D) Adding a decoder for generation

---

**Q11. The GPT series (GPT-1 → GPT-2 → GPT-3) primarily demonstrated the power of:**

A) Scaling parameters and data, leading to emergent few-shot and zero-shot capabilities  
B) Reducing model size for efficiency  
...
C) Training a GAN on word pairs  
D) Looking up pre-computed word vectors
...
C) Using only a decoder without any encoder  
...
A) Using character-level instead of subword tokenization  
B) Cross-layer parameter sharing and factorized embedding parameterization  
C) Reducing the training corpus size  
...
B) They require too much memory  
C) They assign the same vector to a word regardless of context, failing to capture polysemy (e.g., "bank" as financial vs. river)  
D) They cannot represent common words
