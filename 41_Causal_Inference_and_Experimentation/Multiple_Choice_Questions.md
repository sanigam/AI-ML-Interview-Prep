...
A) The sample size is too small to detect a true effect  
B) The company failed to use randomization to establish causation  
C) The correlation observed could be confounded by customer engagement—more engaged customers are both more likely to download the app and more likely to spend money regardless  
...
A) The difficulty of designing experiments in real-world business settings  
B) The inability to collect enough data for statistical significance  
C) The challenge of controlling for all possible confounding variables  
D) The impossibility of observing both potential outcomes Y(1) and Y(0) for the same unit simultaneously
...
B) Average Treatment Effect on the Control (ATC)  
C) Heterogeneous Treatment Effect (HTE)  
D) Average Treatment Effect on the Treated (ATT)
...
A) The operating system employees use (independent of both treatment and outcome)  
B) Total hours worked, which is directly caused by the software  
C) The time employees have used the software (mediator)  
D) Employee motivation level, which affects both whether they adopt the software and their output
...
A) Don't condition on any variables; just compare T and Y  
B) Condition on X to control for confounding  
C) Condition on both X and M  
D) Condition on M to block the indirect path
...
B) Unconfoundedness—all variables that confound treatment and outcome are observed and included  
C) All units within propensity score quintiles are perfectly homogeneous  
D) The propensity score model must have higher predictive accuracy than the outcome model
...
A) It eliminates the need for checking covariate balance  
B) It doesn't require the unconfoundedness assumption  
C) It automatically handles unmeasured confounding  
D) It uses all observed data instead of discarding unmatched units, making it more statistically efficient
...
A) When you believe there are unmeasured confounders affecting treatment and outcome  
B) When treatment is randomly assigned in the data  
C) When you have a large sample size (n > 10,000)  
...
A) Treatment and control stores must be in the same geographic region  
B) Absent treatment, the treatment group would follow the same trajectory as the control group  
C) The treatment effect must be constant for all stores  
D) Treatment and control stores must have identical sales levels before the treatment begins
...
A) Because larger sample sizes near the threshold ensure statistical power  
B) Because the running variable is truly random near the threshold  
C) Because units on either side of the threshold are expected to be similar in all characteristics except treatment assignment, assuming continuity of the outcome function  
D) Because the RDD method automatically removes all confounding variables
...
B) That correlation never implies causation under any circumstances  
C) That associations can reverse direction depending on which variables you condition on, and proper conditioning requires causal reasoning  
D) That randomization is always inferior to observational methods
...
A) P(Y | T=1) requires a randomized experiment while P(Y | do(T=1)) works with observational data only  
B) P(Y | T=1) is causal while P(Y | do(T=1)) includes bias from selection effects  
C) P(Y | T=1) includes selection bias from confounding while P(Y | do(T=1)) isolates the causal effect of forcibly setting T=1  
D) They are mathematically identical; the notation is just stylistic
...
B) Because it makes the unconfoundedness assumption unnecessary  
C) Because it automatically increases the sample size in experiments  
D) Because it eliminates the need for any control group in experiments
...
A) It becomes impossible to calculate  
B) It stays at the nominal α = 0.05  
C) It increases above α = 0.05 due to multiple testing implicit in sequential checking  
D) It decreases below α = 0.05 due to additional data analysis
...
A) Peeking; solve by pre-registering the analysis plan  
B) Network effects / interference; solve by randomizing at the cluster level (entire friend group) rather than individual level  
C) Sample ratio mismatch; solve by checking if randomization broke  
D) Novelty effects; solve by running the test longer
...
**Q6: B** - Propensity score matching requires unconfoundedness: all confounders are observed and included in the propensity score model. Without this, matching fails to control hidden confounders. Common support (A) is also necessary but not sufficient. Predictive accuracy (B) is less important than covariate balance. Perfect homogeneity (D) is impossible.

**Q7: D** - IPW uses all data points by reweighting rather than discarding unmatched units as matching does. This improves statistical efficiency and power. It does NOT relax unconfoundedness (A), automatically handle unmeasured confounding (D), or eliminate balance checks (C). All require the same core assumptions.

**Q8: A** - IV handles unmeasured confounding by using an exogenous source of variation (the instrument) to extract treatment effect, unlike propensity score methods which require measured confounders. Large sample size (A) alone doesn't necessitate IV. Random assignment (C) makes IV unnecessary. HTE estimation (D) is orthogonal to the IV vs. propensity score choice.
