...
(2) find all paths from treatment to outcome,

(3) identify backdoor paths (those entering treatment from the back), and (4) condition on or stratify by variables that block only backdoor paths. This principled approach prevents over-conditioning (which introduces bias) or under-conditioning (which leaves confounding).
...
(2) it only controls for measured confounders, not unmeasured ones;

(3) matching discards unmatched units, reducing sample size and generalizability;
...
(2) checking covariate balance before and after weighting (standardized mean differences should be < 0.1);

(3) using logistic regression as baseline, but also trying flexible models (boosting, random forests) if linear assumptions fail;
...
(2) excluding periods with confounding events that differ between groups,

(3) testing robustness with alternative control groups. The advantage is that DiD requires weaker assumptions than cross-sectional matching (no need to condition on confounders, only that they affect groups proportionally).

Limitations: it requires repeated observations over time, parallel trends may fail if treatment groups experience different macro shocks, and treatment effects may vary over time (time-varying treatment effects require extensions).
...
(1) fork (Z ← X → Y, where X confounds Z and Y), blocked by conditioning on X;
...
(2) faithfulness (only the independencies in the DAG hold in the data, no accidental cancellations);

(3) no cycles;

(4) sometimes additional assumptions like functional constraints or temporal ordering. These are often unrealistic, so causal discovery is highly sensitive to violations. In practice, you need domain knowledge to specify: which variables might have causal relationships, temporal ordering of variables, which relationships are definitely causal or definitely absent. The workflow is:

(1) domain experts specify plausible causal structures,

(2) algorithms validate against conditional independencies in data,

(3) sensitivity analysis explores variations. Pure algorithmic causal discovery from data is risky and leads to misleading conclusions; it's best used as a hypothesis generation tool combined with domain expertise and causal reasoning.
...
(2) x-learner (transforms outcome to deviations from group means then trains on treatment/control residuals), more stable for small samples;

(3) causal forests (random forests with modifications to ensure valid inference). The advantage is precision targeting—marketing budgets improve when spent on high-uplift individuals instead of spraying broadly. Challenges include:

(1) individual effects are noisier than average effects (much more data needed for precision),

(2) strong assumptions required for valid HTE estimates (unconfoundedness in observational data is harder to satisfy),

(3) model heterogeneity often dominates (capturing which customers respond is hard). Use uplift modeling for resource-constrained decisions (can't treat everyone) or harm avoidance (negative uplift costs more). Always validate heterogeneous effects on holdout test data; uplift model overfitting is common because individual effects are inherently noisier.
...
(2) sufficient sample size per power analysis,

(3) appropriate randomization unit,

(4) long run duration,

(5) continuous monitoring of assignment ratio and data quality. Combine frequentist rigor with Bayesian thinking about prior odds and cost-benefit analysis for business decision-making.
