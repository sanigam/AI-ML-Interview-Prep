...
A) Because the batch size is fixed at 140 requests  
B) Because each parameter requires 2 bytes of storage and 70B × 2 bytes = 140GB  
C) Because the KV-cache is stored separately from model weights  
D) Because attention matrices scale quadratically with sequence length

---

**Q2. Which of the following is NOT a primary cost factor in LLM inference optimization?**

A) Latency per token generation  
B) The color of the GPU hardware  
C) Model memory consumption  
D) Compute cost proportional to tokens × model size

---

**Q3. What is the key difference between INT8 and INT4 quantization?**

A) INT4 has zero accuracy loss while INT8 loses 10-20% accuracy  
B) INT8 uses 8-bit integers while INT4 uses 4-bit integers; INT4 achieves ~99.2% size reduction vs. FP32, while INT8 achieves ~96.9%  
C) INT8 is slower than INT4 on all hardware  
...
A) The Hessian of the loss (curvature/sensitivity)  
B) The magnitude of each weight  
C) The layer number  
...
A) Structured pruning because it removes entire components (heads, layers) that hardware can efficiently skip  
B) Unstructured pruning because all GPUs have sparse matrix acceleration  
C) Both are equally effective; the choice depends only on model size  
D) Unstructured pruning because it achieves higher compression ratios
...
B) By always accepting the draft model's tokens  
C) By comparing target probability to draft probability; accepting tokens if P_target > draft probability, ensuring the output distribution matches the target exactly  
...
A) Without caching, generating token tᵢ requires O(i) recomputation of attention; caching reduces this to O(1)  
B) The KV-cache itself uses too much memory  
...
A) By using only FP8 precision for all computations  
B) By reducing the attention matrix size  
C) By storing KV-cache in fixed-size blocks/pages and enabling memory reuse across requests with shared prefixes  
D) By eliminating the need for KV-cache entirely

---

**Q10. In continuous batching vs. static batching, which statement is true?**

A) Static batching always achieves lower latency  
B) Both have identical throughput for variable-length requests  
C) Continuous batching requires more model parameters  
D) Static batching waits for a full batch before processing, while continuous batching processes requests as they arrive and removes completed requests immediately
...
A) Pipeline parallelism is always superior  
B) When maximizing batch size is the primary goal  
C) When you have 2-4 GPUs and prioritize low latency; communication overhead is manageable at small scales  
D) When you have 50+ GPUs
...
A) 10-20x less memory by computing attention in blocks without storing the full O(seq_len²) matrix  
B) 100% memory savings — it uses zero memory  
C) No memory savings; only computation savings  
D) 5-10% reduction
...
A) TTFT = latency from request to first token (prefill-dominated); ITL = time between successive tokens (decoding-dominated)  
B) TTFT and ITL are identical metrics with different names  
C) TTFT is the total time to generate all tokens; ITL is the time per token  
D) ITL is only relevant for batch inference
...
A) Request fallback (escalating to larger model if smaller model has low confidence)  
B) Prefix caching  
C) Spot instances for batch processing  
D) Increasing the model size for all requests

---

**Q15. Which of the following statements about edge deployment (phones/IoT) of LLMs is most accurate?**

A) Edge deployment is now practical for 70B models without any optimization  
B) CPU inference on edge devices is as fast as GPU inference  
C) Edge deployment uses the same models and techniques as data center inference  
D) Edge deployment requires extreme optimization: aggressive quantization (INT4), knowledge distillation, structured pruning, and typically smaller base models (3-7B) to fit memory constraints and latency budget
