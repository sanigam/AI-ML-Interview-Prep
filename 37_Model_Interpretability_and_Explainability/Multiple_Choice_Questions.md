...
B) Explainability is stricter than interpretability and always requires regulatory compliance  
C) Interpretability applies only to neural networks, while explainability applies to traditional models  
D) Interpretability and explainability are interchangeable terms referring to the same concept of understanding model decisions
...
A) They are the only attribution method satisfying local accuracy, symmetry, and dummy properties simultaneously  
B) They automatically handle categorical features without preprocessing  
C) They are the fastest method to compute across all model types  
...
A) O(leaf_count * num_features) instead of exponential  
B) No improvement; it only provides approximate values faster  
C) Linear in the number of features instead of exponential  
D) Logarithmic instead of polynomial
...
A) LIME eliminates the need for Shapley values or other feature attribution methods  
...
D) LIME produces stable explanations that remain consistent across multiple runs on the same instance
...
A) KernelSHAP produces exact values but requires retraining the model  
B) KernelSHAP is model-agnostic and works for any model, but is slower and produces approximate (not exact) Shapley values  
C) KernelSHAP requires fewer model evaluations but produces lower-quality approximations  
D) KernelSHAP is faster but only works with tree-based models
...
A) Attention weights don't directly measure feature importance; high attention doesn't necessarily mean the token is important for the decision  
B) Transformer models don't actually use attention mechanisms  
C) Attention weights show what the model attends to, which always correlates with feature importance for the final prediction  
D) Attention weights are too expensive to compute for large models
...
A) Drop-column importance is specific to tree models, while permutation importance works for all models  
B) Permutation importance retrains the model without the feature, while drop-column importance shuffles the feature values  
C) Permutation importance shuffles feature values to measure performance drop, while drop-column importance retrains the model entirely without the feature  
...
C) PDPs cannot be used with categorical features  
D) PDPs require the model to be a linear model
...
A) ICE curves replace the need for PDPs in modern interpretability practices  
...
C) ICE curves are only applicable to tree-based models  
D) ICE curves provide faster computation than PDPs
...
A) They answer "what needs to change for a different decision?" which is actionable and intuitive, unlike abstract importance scores  
B) They work equally well for all types of models without any additional considerations  
C) They completely eliminate the need for regulatory compliance documentation  
D) They are the most accurate method for computing feature importance
...
A) Local explanations are used for debugging, global explanations are only for regulatory compliance  
B) The choice between global and local explanations has no practical difference  
C) Global explanations should always be used instead of local explanations  
D) Global explanations help identify systematic issues across the dataset like bias patterns, while local explanations justify individual decisions
...
A) The right to explanation only applies to recommendation systems, not credit or hiring decisions  
B) Companies must always use only interpretable models like linear regression  
C) Meaningful information about the logic and significance of the decision; this can be satisfied through interpretable models, explainability methods, or human review  
D) Companies must explain every single model prediction in real-time to users
...
B) Use only neural networks since they provide the best interpretability through attention mechanisms  
C) Start with interpretable baseline models and add complexity only if accuracy gains justify it; use explainability methods as a fallback  
D) Ignore interpretability completely and focus solely on predictive performance
