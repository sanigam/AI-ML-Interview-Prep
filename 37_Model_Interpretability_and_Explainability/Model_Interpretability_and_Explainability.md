...
**A:** LIME explains a model's prediction by fitting a simple local model (typically linear) around the prediction instance. Process:

(1) Generate perturbed versions of the input (e.g., turn random pixels on/off for images),

(2) Get predictions from the complex model for each perturbed input,

(3) Fit a weighted linear model to predict perturbed → actual predictions, weighting samples by proximity to original input. The linear model's coefficients show feature importance locally—which features matter for this specific prediction? LIME works for any model and is model-agnostic. Strength: easy to understand (linear model coefficients are intuitive), works for images and text.

Limitations:

(1) Instability—small changes to the instance can cause large changes in LIME explanations,

(2) Linear assumption—assumes the model behaves linearly around the instance, which may not hold for complex decision boundaries,

(3) Computational cost—generating and predicting on thousands of perturbed samples is slow,

(4) Arbitrary perturbing—how you generate perturbations (e.g., which pixels to turn off) affects explanations.

Example: Explaining why an image classifier predicted "cat": LIME turns on/off random regions of the image, finds that regions containing the cat's head and body are most important for the "cat" prediction, and highlights them. However, LIME might highlight the couch in the background if the model is biased toward it, and different runs might highlight slightly different regions due to randomness. Modern alternatives like SHAP address some of these limitations with theoretical grounding.

---

### Q5: Describe how SHAP values are computed for tree models using TreeSHAP.

**A:** TreeSHAP efficiently computes exact Shapley values for tree models (decision trees, random forests, gradient boosting) by exploiting tree structure instead of expensive coalition enumeration. For a specific prediction, TreeSHAP traces the sample down the tree, identifying which features were actually used in the path.

Key insight: when a sample reaches a leaf, not all features mattered—only features on the decision path matter. TreeSHAP computes each feature's contribution by considering how much the prediction would change if that feature were excluded from the tree. Mathematically, TreeSHAP efficiently sums over all possible feature orderings using dynamic programming on tree structure. Computational complexity: O(leaf_count * num_features) instead of exponential, making exact Shapley values practical for tree models.
...
**A:** KernelSHAP (SHAP using kernel method) makes Shapley value computation practical for any model by fitting a weighted linear regression locally. Process:

(1) Sample coalition vectors (each row is a binary vector indicating which features are "present"),
...
Limitations: these methods show what the model paid attention to, but don't explain why—a highlighted region might not be sufficient to understand the decision. Grad-CAM is standard in medical AI to ensure models are learning appropriate visual features rather than dataset artifacts.
...
(1) Attention patterns are learned to optimize training loss, not interpretability, so high attention doesn't necessarily mean the token is important for the final decision.

(2) Multi-head attention means different heads attend differently, and their contributions vary.

(3) Attention in early layers is different from attention in late layers—early attention might focus on syntax, late attention on semantic relationships.

(4) Gradient-based importance (how much does changing this token affect output?) doesn't correlate perfectly with attention.

Example: In question-answering, the model might attend heavily to a pronoun (because it's next to the answer), but the pronoun itself isn't important—it's important because of its context. Attention visualization in NLP often shows patterns like tokens attending to themselves or adjacent tokens, which looks trivial but isn't actually uninformative. Better approaches: combine attention visualization with gradient-based importance or perturbation analysis. Some recent work shows that attention visualizations are misleading for interpretability, and alternatives like gradient-based methods provide more reliable importance measures.

---

### Q9: Compare permutation importance, drop-column importance, and gain-based importance for feature importance.

**A:** Permutation importance measures how much model performance drops when a feature's values are randomly shuffled (breaking correlation with target), suggesting feature importance. Algorithm:
...
**A:** Partial Dependence Plots (PDP) show the relationship between a feature and model prediction, marginalized over other features. To create a PDP:

(1) For each value v of feature X,
...
(2) Use explainability methods (SHAP, LIME) providing post-hoc explanations,
