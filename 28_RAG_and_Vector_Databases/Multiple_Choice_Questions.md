...
A) Training on only verified facts  
B) Removing the attention mechanism  
C) Retrieving relevant documents and including them in the prompt, grounding generation in actual sources  
D) Making the model smaller

---

**Q2. The standard RAG pipeline consists of:**

A) Only document storage  
...
A) The cosine of the angle between them, capturing directional similarity regardless of magnitude  
B) The product of their norms  
C) Whether they have the same number of dimensions  
D) The Euclidean distance between them
...
A) It splits documents into retrievable units of appropriate size, balancing retrieval precision and context completeness  
B) It makes documents longer  
...
B) No difference compared to large chunks  
C) Higher precision but risk losing context; larger chunks provide more context but may include irrelevant information  
D) Always better performance than large chunks

---

**Q7. HNSW (Hierarchical Navigable Small Worlds) indexing provides:**

A) Approximate nearest neighbor search in O(log n) time using a hierarchical graph structure  
B) Exact nearest neighbor search in O(1)  
C) O(n) brute-force search  
...
A) Using only the first dimension  
B) Deleting half the vectors  
C) Compressing vectors by splitting them into subspaces and quantizing each to fewer bits  
D) Storing full-precision vectors

---

**Q9. Hybrid search combines dense (embedding) and sparse (BM25) retrieval because:**

A) Dense captures semantic similarity while sparse captures exact keyword matching, and combining them improves overall recall  
B) They use the same algorithm  
C) Dense retrieval always outperforms sparse  
D) Sparse retrieval is no longer useful

---

**Q10. Re-ranking in a RAG pipeline improves results by:**

A) Randomly shuffling documents  
B) Adding more documents to the index  
C) Using a more expensive cross-encoder model to re-score and reorder the initially retrieved candidates  
D) Removing the retrieval step entirely

---

**Q11. The embedding model used in RAG should be chosen based on:**

A) Domain relevance, speed, dimensionality, and quality on retrieval benchmarks (e.g., BEIR)  
B) Only its parameter count  
...
B) Train a new model  
C) Remove duplicate documents only  
D) Merge ranked results from multiple retrieval systems using a simple formula based on rank positions

---

**Q13. A key limitation of RAG is:**

A) It cannot use any external documents  
B) If the retrieved documents don't contain the answer, the LLM may still hallucinate or produce low-quality responses  
C) It requires retraining the LLM for each query  
D) It always produces perfect answers
...
B) HNSW cannot handle high-dimensional data  
C) They are identical in all aspects  
D) IVF has lower memory overhead and faster build time; HNSW has faster query time and higher recall but uses more memory
